...
The `grounded-ui` React frontend provides a split-pane interface: the left panel shows the answer and result table, the right panel exposes the full execution internals via three tabs.

**Empty state** — query input with example prompts, Trace/Plan/SQL panel on the right:
...
The dev server proxies `/analytics/*` to `http://localhost:5252` by default. Set `VITE_API_BASE_URL` to override.
